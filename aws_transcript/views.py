...
import pandas as pd
from aws_transcript.helper import aws_transcribe,get_stutter_info,get_cleaned_ans,get_cleaned_transcript,check_grammer_error
from aws_transcript.models import HiringTranscript, HiringWord,HiringQuestionAnswer,HiringRelevance,HiringVideoIdToVideoTypeMaping,HiringOkGrammerErrorList,HiringNotOkGrammerErrorList,HiringReiviewGrammerErrorList
from aws_transcript.serializers import TranscriptSerializer, WordSerializer, TranscriptEditSerializer,FileUploadSerializer,RelevanceSerializer
...
class import_question_and_data(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for ind,row in df_question_ans.iterrows():
            ...
            QA_data=HiringQuestionAnswer(video_type=row['video_type'],video_type_id=row['video_type_id'],questionID=row['questionID'],question=row['question'],answer=row['answer'])
            QA_data.save()
        content = {
...
class FillRelevanceTable(generics.CreateAPIView):
    def get(self,request,*args,**kwargs):
        ...
        for transcript in transcript_datas:
            ...
            relven.save()
            relven.video_type_id=HiringVideoIdToVideoTypeMaping.objects.get(video_type=transcript.video_type).video_type_id
